/*  Pricing engine — WITH defer (4D 21 R4).

	Exactly the same logic, the same benchmark and the same twelve exit
	points. The measure is closed ONCE, on the line right below the one that
	started it, and 4D closes it whatever the exit is.

	1 defer line instead of 12 — and no measure can ever be lost.
*/
